You are running the AI Reader pipeline. This routine fires once per day; ship items that are new since the last run.
...
CITATION_SOURCES = ["Ethan Mollick", "Andrej Karpathy", "The AI Daily Brief", "Everyday AI", "Latent Space"]
...
4. Read `profile.md` and `candidates.json`. The reader is a senior pharma IT exec leading the AI strategic pillar in Prague. He listens to **Latent Space**, **The AI Daily Brief** (Whittemore), **Everyday AI** (Wilson), and reads **Ethan Mollick** and **Andrej Karpathy**. Pick the way they pick.
...
5. **Cite-worthiness signals.** A candidate scores higher when it carries one or more of these:
   ...
   - **Author from CITATION_SOURCES** — Mollick, Karpathy, Daily AI Brief, Everyday AI, Latent Space. +0.5 implicit bonus because their cited content seeds further saves.
...
7. **Pick up to 7 candidates with weighted score ≥ 5.5.** If fewer than 3 qualify, reply "Nothing worth shipping this run." and stop. Aim for spread across at least three clusters: frontier model moves, enterprise deployment, capital flows/compute, agentic systems, governance/workforce, AI-for-science, builder methodology. For each pick write a single sentence (max 25 words) explaining why it matters.

8. Write a **120–180 word editor's note** connecting the picks. Direct, peer-to-peer, no throat-clearing. Lead with the strongest strategic thread.

9. **Compose the brief HTML** — a self-contained page:
...
12. **Save each picked article.** For each pick, POST https://readwise.io/api/v3/save/ with body:
    ...
    Sleep 200ms between calls. Track 2xx responses; failures logged to stderr.
...
13. **Extract and save citations** — for each successfully-saved pick whose source is in CITATION_SOURCES:
    ...
    b. Identify the **1–3 most substantive externally-cited items** (arXiv links, GitHub repos with research/academic orientation, named research papers or blog posts the author references substantively). **Skip**: social-media links (twitter/x.com, threads, mastodon), the source site's own related-content links, navigation, footnote ornaments, image-only embeds, podcast platform links.
